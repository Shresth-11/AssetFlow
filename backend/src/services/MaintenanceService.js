...
class MaintenanceService {
  static async listRequests(userRole, userId, userDeptId, filters = {}) {
  ...
  static async raiseRequest(data, user) {
  ...
  static async assignTechnician(id, data, userId) {
    return db.transaction(async (trx) => {
      const request = await trx("maintenance_requests").where({ id }).forUpdate().first();
      if (!request) {
        throw new Error("Maintenance request not found");
  ...
  static async resolveRequest(id, userId) {
    return db.transaction(async (trx) => {
      const request = await trx("maintenance_requests").where({ id }).forUpdate().first();
      if (!request) {
        throw new Error("Maintenance request not found");
  ...
  static async rejectRequest(id, userId) {
    return db.transaction(async (trx) => {
      const request = await trx("maintenance_requests").where({ id }).forUpdate().first();
      if (!request) {
        throw new Error("Maintenance request not found");
