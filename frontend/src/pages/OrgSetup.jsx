...
import { Plus, Trash2, UserMinus, Sparkles } from "lucide-react";
...
export const OrgSetup = () => {
  ...
  const handleRemoveTempField = (key) => {
    setTempFields((prev) => {
      const updated = { ...prev };
      delete updated[key];
      return updated;
  ...
  const handleRolePromotion = async (empId, targetRole) => {
    if (empId === user?.id) {
  ...
  const handleStatusToggle = async (empId, targetStatus) => {
    if (empId === user?.id) {
